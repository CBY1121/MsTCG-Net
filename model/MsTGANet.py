...
# from model.MsGCS import MsGCS as MsM
from model.MsTNL import MsTNL
...
class SPP_q(nn.Module):
    def __init__(self,in_ch,out_ch,down_scale,ks=3):
        ...
        self.UP = nn.Upsample(scale_factor=down_scale,mode="bilinear")  # scale_factor：指定输出为输入的多少倍数
...
class MsTGANet(nn.Module):
    def __init__(self, in_channels=1, num_classes=1, feature_scale=2):
        ...
        self.SPP_q_0 = SPP_q(in_ch=n_dims, out_ch=Filters[0], down_scale=2, ks=3)
        self.SPP_q_1 = SPP_q(in_ch=n_dims, out_ch=Filters[1], down_scale=4, ks=3)
        self.SPP_q_2 = SPP_q(in_ch=n_dims, out_ch=Filters[2], down_scale=8, ks=3)
        self.SPP_q_3 = SPP_q(in_ch=n_dims, out_ch=Filters[3], down_scale=16, ks=3)
