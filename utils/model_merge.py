...
def merge_roc_octa():
    ...
    path = './assets/'
    path1 = path + 'octa/'
    fpr1, tpr1, auc1, avg_auc1 = roc_auc(path1, dir='pred/')
    #print(fpr1)
    #print(tpr1)
    print(auc1)
    print('average auc is {}'.format(avg_auc1))
    path2 = 'D:/OCT/OCTA/CS-Net/Rose_Unet/UNet-rose/assets/octa/'
    fpr2, tpr2, auc2, avg_auc2 = roc_auc(path2, dir='pred/')

    plt.figure()
    plt.plot(fpr1, tpr1, label='CSANet(auc={0:.4f})'.format(avg_auc1), color='red', linestyle='-', linewidth=1)
    plt.plot(fpr2, tpr2, label='UNet(auc={0:.4f})'.format(avg_auc2), color='green', linestyle='-', linewidth=1)
    plt.xlim(-0.002, 1.)
    plt.ylim(0., 1.005)
